...
function updateBooleanFieldInfo($table,$field,$attribute)
{
    ...
    {
        $g_def[$table] ['fields'] [$field]  [$attribute] = false;
    }
}
            ...
            if (isset($_POST["{$table}_{$field}_type"]) && $_POST["{$table}_{$field}_type"] == "choice")
            {
                ...
                for ($cnt = 0; $cnt < 50; $cnt++)
                {
                    if (isset($_POST["{$table}_{$field}_form_choice_text{$cnt}"]) )
                    {
                        ...
                        array_push($g_def[$table] ['fields'] [$field] ['form'] ['choice'] ,$ch);
                    }
                }
            }
        }
...
?></div>
                <?php
                    if ($g_field)
                    {
                        ...
                        if (! isset($g_def[$g_table] ['fields'] [$g_field] ['readonly'] ) )
                        {
                            $g_def[$g_table] ['fields'] [$g_field] ['readonly'] = false;
