...
class geoConversion {
    ...
    static convertTodm(v, islat) {
        ...
        let d = parseInt(v);
        let p = parseFloat((v - d) * 60).toFixed(3);
        let s = d + " " + p;
        if (neg) 
            s += islat ? " S" : " W";
        else 
    ...
    static convertTodms(v, islat) {
        ...
        let d = parseInt(v);
        let e = (v - d) * 60;
        let p = cv.pad(parseInt(e), 2);
        let q = parseFloat((e - p) * 60).toFixed(2);
        let s = d + " " + p + " " + q;
        if (neg)
            s += islat ? " S" : " W";
        else
    ...
    static test() {
        console.log("Testing lat/lon -41.344071, 174.742869");
